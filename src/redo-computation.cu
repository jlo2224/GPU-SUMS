...
__global__ void kernel2(int *arr, int *d_n, int *d_ws);
...
void inclusive_scan(int *arr, int n, int *result)
{
    ...
    int ws_size = blocks + 2 * ((blocks + threads * widthFactor - 1) / (threads * widthFactor));
    ...
    kernel2<<<blocks, threads>>>(d_arr, d_n, d_ws);
    cudaDeviceSynchronize();
...
__global__ void prefix_sum(int *arr, int *n, int *ws)
{
    ...
    heap[2 * threadIdx.x] = 0;     // just added
    heap[2 * threadIdx.x + 1] = 0; // just added
    __syncthreads();
    for (unsigned char i = 0; i < widthFactor; i++)
    {
        heap[threadIdx.x + threads] += arr[blockIdx.x * threads * widthFactor + threadIdx.x * widthFactor + i];
    }
    __syncthreads();
    if (threadIdx.x % 2 == 0)
    {
    ...
    for (unsigned char i = 0; i < logthreads; i++)
    {
        ...
        if (threadIdx.x % pow2 == 0)
        {
            ...
            destination /= 2;              // reassign for next layer
        }
        pow2 *= 2;
    }
    if (threadIdx.x == 0)
...
__global__ void kernel2(int *arr, int *d_n, int *ws)
{
    __shared__ int heap[2 * threads];
    int left, right, pow2, destination;
    heap[2 * threadIdx.x] = 0; // try changing to see if bank conflicts
    heap[2 * threadIdx.x + 1] = 0;
    ...
    for (unsigned char i = 0; i < logthreads; i++)
    {
        ...
        {
            heap[(start + threadIdx.x * start / threads) * 2 + 1] = heap[start + threadIdx.x * start / threads];
        }
        __syncthreads(); // remove and rejoin ifs
        if (threadIdx.x % (threads / start) == 0)
        {
            if (threadIdx.x * start / threads + 1 != start)
            {
                heap[(start + threadIdx.x * start / threads) * 2 + 2] += heap[start + threadIdx.x * start / threads];
            }
        }
        start *= 2;
        __syncthreads();
    ...
    if ((int)blockIdx.x > 0)
    {
        arr[blockIdx.x * threads * widthFactor + threadIdx.x * widthFactor] += ws[blockIdx.x - 1];
    }
    if (threadIdx.x != 0)
    {
        arr[blockIdx.x * threads * widthFactor + threadIdx.x * widthFactor] += heap[threads + threadIdx.x - 1]; // optimization: eliminate this if statement, by allocating 513 spaces for the heap, so index 256 holds the value 0
    }
    __syncthreads(); // remove
    for (unsigned char i = 1; i < widthFactor; i++)
    {
        arr[blockIdx.x * threads * widthFactor + threadIdx.x * widthFactor + i] += arr[blockIdx.x * threads * widthFactor + threadIdx.x * widthFactor + i - 1];
        __syncthreads(); // remove
    }
}
...
__global__ void kogge_stone(int *write_start, int *ws, int *offset_access)
{
    ...
    if (threadIdx.x == 0)
    {
        if (blockIdx.x * threads * widthFactor + threads * widthFactor + offset <= limit) // something might be wrong here
        {
            ws[limit + blockIdx.x] = arr[threads * widthFactor - 1];
        }    /// idk what i wrote here but it works
        else // want to find the largest place in arr[]
        {
