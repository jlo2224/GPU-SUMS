...
void inclusive_scan(int *arr, int n, int *result)
{
    ...
    add_global_aggregates<<<blocks - 1, threads>>>(d_arr, d_n, d_ws);
    cudaDeviceSynchronize();
...
{
    __shared__ int aggregates[widthFactor * threads / 32]; // technically 32 should be warpSize, but we want to statically allocate memory
    int laneID = threadIdx.x % warpSize;
    int warpID = threadIdx.x / warpSize;
    int start_index_for_warp = blockIdx.x * threads * widthFactor + widthFactor * warpSize * warpID;
    int local_sum = 0;
    int val;
    int add;
    ...
    ws[blockIdx.x] = aggregates[warpSize - 1];
}
...
__global__ void prefix_sum(int *arr, int *n, int *ws)
{
    ...
    if (warpID > 0) // take a different approach, as this causes bank conflicts
    {
        for (unsigned char i = 0; i < widthFactor; i++)
        {
            arr[start_index_for_warp + i * warpSize + laneID] = values[i] + aggregates[widthFactor * warpID + i - 1];
        }
    }
    else
    {
        arr[start_index_for_warp + laneID] = values[0];
        for (unsigned char i = 1; i < widthFactor; i++)
        {
            arr[start_index_for_warp + i * warpSize + laneID] = values[i] + aggregates[i - 1];
        }
        if (threadIdx.x == 0)
            ws[blockIdx.x] = aggregates[warpSize - 1];
    }
...
__global__ void add_global_aggregates(int *arr, int *d_n, int *ws)
{
    ...
    int start_index_for_warp = (1 + blockIdx.x) * threads * widthFactor + widthFactor * warpSize * warpID;
    for (unsigned char i = 0; i < widthFactor; i++)
