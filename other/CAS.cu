...
void inclusive_scan(int *arr, int n, int *result)
{
    ...
    ws_size = 2 * (blocks + 2 * ((blocks + threads * widthFactor - 1) / (threads * widthFactor)));
...
__global__ void downsweep(int *arr, int *d_n, int *ws) // some access of shared memory has bank conflicts
{
    ...
    int i = 1;
    __shared__ int kernel_level_sum;
    if (threadIdx.x == 0)
    {
        ws[2 * blockIdx.x + 1] = A;
    ...
    while (i != 0 && threadIdx.x == 0)
    {
        if (ws[2 * (blockIdx.x - i) + 1] == A)
        {
            kernel_level_sum += ws[2 * (blockIdx.x - i)];
            i++;
        }
        else // if (ws[2 * (blockIdx.x - i)] == P)
        {
            kernel_level_sum += ws[2 * (blockIdx.x - i)];
            int64_t write_value = ((int64_t) (ws[2 * blockIdx.x] + kernel_level_sum) << 32) | P;
            if (threadIdx.x == 0)
            {
                atomicCAS(ws + 2 * blockIdx.x, * (ws + 2 * blockIdx.x), write_value);
            }
            break;
        }
    }
